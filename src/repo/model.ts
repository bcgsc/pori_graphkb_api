...
import { IndexType } from '@bcgsc-pori/graphkb-schema/dist/types';
 ...
 */
const createPropertyInDb = async (model: kbSchema.Property, dbClass: orientjs.dbClass) => {
    const dbProperties: {
        mandatory: boolean;
        name: string;
        notNull: boolean;
        ...
        mandatory: Boolean(model.mandatory),
        name: model.name,
...
const checkDbCanCreateIndex = (model: kbSchema.ClassModel, index: IndexType): boolean => {
 ...
 */
const createClassModelInDb = async (model: kbSchema.ClassModel, db: orientjs.Db, opt: {properties?: boolean; indices?: boolean; graceful?: boolean;} = {}) => {
    const {
        properties = true,
        indices = true,
        graceful = false,
    ...
    if (indices) {
        ...
        await Promise.all(
            model.indices
                .filter((i) => checkDbCanCreateIndex(model, i))
                .map(createIndex),
        );
    }
    return cls;
 ...
 */
 const compareToDbClass = (model: kbSchema.ClassModel, oclass: orientjs.dbClass) => {
